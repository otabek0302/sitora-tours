  ...
  return (
    <div key={city.id} className='embla__slide w-full flex-shrink-0 lg:w-[calc(25%-18px)]'>
      <Link href={`/cities/${city.slug}`}>
        <div className='bg-card border-border group relative h-[300px] cursor-pointer overflow-hidden rounded-[26px] border shadow-none transition-all duration-300 hover:shadow-lg hover:scale-[1.02] lg:h-[480px]'>
          {/* City Image */}
          {city.image?.url ? (
          ...
          <div className='text-sitora-white absolute right-0 bottom-0 left-0 p-6'>
            ...
            <p className='text-sm font-normal opacity-90 transition-all duration-300 group-hover:opacity-100'>
              ...
              {city.description && city.description.length > 100 && (
                <span className='hidden group-hover:inline'>{city.description}</span>
              )}
            </p>
          </div>
        </div>
