...
import { Button, RecommendedCitiesCard } from '@/components/ui'
import { usePagesContext } from '@/lib/stores/pages'

const RecommendedCities = () => {
  const t = useTranslations('pages.home.recommended_cities')
  const { getRecommendedCitiesSection } = usePagesContext()
  const { cities = [] } = getRecommendedCitiesSection() || {}
      ...
      <div className='container mx-auto px-4 sm:px-6 lg:px-8'>
        ...
        <div className='relative'>
          <div className='embla' ref={emblaRef}>
            <div className='embla__container flex gap-4 py-4 pl-1'>{Array.isArray(cities) && cities.filter(city => typeof city !== 'number' && city !== null).map(city => <RecommendedCitiesCard key={city.id} city={city} />)}</div>
          </div>
        </div>
