...
const TourGallery = ({ tour }: TourGalleryProps) => {
  ...
  if (!tour.images?.length) {
    return <div className='bg-sitora-primary-light border-sitora-border h-[300px] w-full rounded-3xl border sm:h-[400px] md:h-[520px]' />
  }

  return (
    <div className='mb-6'>
      <div className='border-sitora-border relative mb-2 h-[300px] w-full overflow-hidden rounded-3xl border sm:h-[400px] md:h-[520px]'>
      ...
      <div className='flex gap-2 overflow-x-auto pb-2'>
        {tour.images?.map((img, idx: number) => (
          <button key={img.id || idx} onClick={() => setMainIdx(idx)} className={`relative h-20 w-32 flex-shrink-0 items-center justify-center overflow-hidden rounded-2xl border-2 transition-all sm:h-24 sm:w-40 md:h-32 md:w-52 ${mainIdx === idx ? 'border-sitora-primary' : 'border-sitora-border'}`} type='button'>
