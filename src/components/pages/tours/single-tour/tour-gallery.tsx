...
const TourGallery = ({ tour }: TourGalleryProps) => {
  ...
  if (!tour.images?.length) {
    return <div className='bg-sitora-primary-light border-sitora-border h-[300px] w-full rounded-[32px] border shadow-none sm:h-[400px] md:h-[600px]' />
  }

  return (
    <div className='mb-6'>
      <div className='border-sitora-border relative mb-2 h-[300px] w-full overflow-hidden rounded-[32px] border shadow-none sm:h-[400px] md:h-[620px]'>
      ...
      <div className='grid grid-cols-6 gap-2 overflow-x-auto pb-2'>
        {tour.images?.map((img, idx: number) => (
          <button key={img.id || idx} onClick={() => setMainIdx(idx)} className={`relative h-20 w-32 flex-shrink-0 items-center justify-center overflow-hidden rounded-[20px] border-2 shadow-none transition-all sm:h-24 sm:w-40 md:h-32 md:min-w-full ${mainIdx === idx ? 'border-sitora-primary' : 'border-sitora-border'}`} type='button'>
