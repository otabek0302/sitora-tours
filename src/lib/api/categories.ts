import { CategoriesResponseSchema, CategorySchema, CategoriesResponse, Category } from '@/lib/schemas'
...
export async function fetchCategoryBySlug(slug: string): Promise<Category> {
  try {
    const response = await apiRequest<any>(`/api/categories?where[slug][equals]=${slug}`)
