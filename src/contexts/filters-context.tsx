...
// Action types
export type FilterAction = { type: 'SET_SELECTED_SERVICE_TYPE'; payload: string | null } | { type: 'SET_SELECTED_CATEGORY'; payload: string | null } | { type: 'SET_DATE_RANGE'; payload: DateRange } | { type: 'SET_PRICE_RANGE'; payload: PriceRange } | { type: 'SET_DURATION_RANGE'; payload: DurationRange } | { type: 'SET_KEYWORD_SEARCH'; payload: string } | { type: 'SET_SELECTED_RATING'; payload: number | null } | { type: 'SET_SELECTED_DIFFICULTY'; payload: string | null } | { type: 'SET_SELECTED_TRANSPORT'; payload: string | null } | { type: 'SET_SELECTED_CAR_TYPE'; payload: string | null } | { type: 'SET_FILTERS_OPEN'; payload: boolean } | { type: 'SET_SORT_BY'; payload: string } | { type: 'SET_VIEW_MODE'; payload: 'grid' | 'list' } | { type: 'RESET_FILTERS' } | { type: 'RESET_ALL_FILTERS' }
...
export function FilterProvider({ children }: { children: ReactNode }) {
    ...
    // Computed values
    const hasActiveFilters = Boolean(state.selectedServiceType || state.selectedCategory || state.dateRange.start || state.dateRange.end || state.priceRange.min > 0 || state.priceRange.max < 10000 || state.durationRange.min > 1 || state.durationRange.max < 30 || state.keywordSearch || state.selectedRating || state.selectedDifficulty || state.selectedTransport || state.selectedCarType)

    const activeFiltersCount = [state.selectedServiceType, state.selectedCategory, state.dateRange.start, state.dateRange.end, state.priceRange.min > 0, state.priceRange.max < 10000, state.durationRange.min > 1, state.durationRange.max < 30, state.keywordSearch, state.selectedRating, state.selectedDifficulty, state.selectedTransport, state.selectedCarType].filter(Boolean).length
